...
2. **Extraction:** For each file with a registered extractor, parse the source and emit `DependencyEdge` objects. 10 extractors cover: Python imports (tree-sitter), JavaScript/TypeScript (ES6 imports, CommonJS require, re-exports via tree-sitter), Go imports (tree-sitter), Docker Compose (YAML-parsed: image refs, depends_on, extends), GitHub Actions (YAML-parsed: uses action/workflow refs), GitLab CI (YAML-parsed: include, extends, needs, trigger, image/service refs), Terraform (regex-based: module source attributes), Helm (template includes, values image refs, Chart.yaml subchart dependencies), Markdown (documentation references: frontmatter depends, inline links, code spans, code block comments).
