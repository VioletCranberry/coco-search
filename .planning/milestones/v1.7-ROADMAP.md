...
Improve search quality with hybrid search (vector + keyword via RRF fusion), context expansion with smart boundaries, symbol-aware indexing with tree-sitter extraction, and full language coverage (31 languages).
...
- RRF k=60 for rank fusion (standard value)
- PostgreSQL 'simple' text config for code identifiers (no stemming)
- Two-phase tsvector: Python preprocessing + PostgreSQL generated column
- Tree-sitter 0.21.x for API compatibility with tree-sitter-languages
- Qualified method names format: ClassName.method_name
- Instance-level LRU cache for search session isolation
- 50-line centering cap for context expansion
- Definition boost (2x) applied after RRF fusion

**Issues Resolved:**

- Hybrid search falling back gracefully for pre-v1.7 indexes
- Symbol extraction error handling (returns NULL, doesn't corrupt index)
- Context expansion performance via LRU cache

**Issues Deferred:**

- Hybrid + symbol filter combination (uses vector-only when both specified)
- Advanced symbol cross-references (count usage)
- Nested symbol hierarchy (fully qualified names with nesting)
