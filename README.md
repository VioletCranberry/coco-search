...
> *Standard code RAG splits files into fixed-size chunks, losing the structure that makes code meaningful. CocoSearch preserves it — [CocoIndex](https://github.com/cocoindex-io/cocoindex) and Tree-sitter provide syntax-aware chunking that keeps functions, classes, and config blocks intact; search results expand to enclosing scope boundaries via Tree-sitter AST; grammar handlers split infrastructure configs at domain-aware boundaries (Terraform resources, CI/CD jobs, Compose services); and a dependency graph maps how files connect across code, config, and documentation. For AI assistants, this means complete, self-contained code units with dependency context in fewer retrieval calls — less guessing about boundaries, less noise in the context window, more accurate answers about how the code actually works.*
