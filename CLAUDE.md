...
- `get_batch_dependencies` — Batch forward dependency query for multiple files. Shared visited set eliminates redundant traversal of overlapping subgraphs. More efficient than per-file calls for git diff analysis.
- `get_batch_impact` — Batch reverse impact query for multiple files. Shared visited set across all roots. More efficient than per-file calls for change blast radius analysis.
